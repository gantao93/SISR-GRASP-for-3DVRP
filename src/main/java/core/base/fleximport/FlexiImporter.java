 ...
 * import of data into the GRASP suite. The general points are
 ...
 * 
 * After import collected data can be accessed GRASP suite.
 */
public class FlexiImporter implements Serializable {
	...
	private ContainerData lastContainerData = null; // null here

	/**
	 * If GRASP suite begins execution, the import process is finished
	...
	public List<Item> getConvertedItemList() {
		List<ItemData> itemListOrder = itemList.stream().sorted(Comparator.comparing(ItemData::getWeight).reversed())
				.collect(Collectors.toList());

//		return itemListOrder.stream()
//				.flatMap(itemData -> {
...
//				.collect(Collectors.toList());
	...
	public List<Container> getConvertedContainerList(List<Item> items, GRASPParameter parameter) {
		boolean isAddingAndRemovingItems = checkForAddRemove(items);
		// order
		List<ContainerData> containerListOrder = containerList.stream().sorted(Comparator.comparing(ContainerData::getVolume))
				.collect(Collectors.toList());
		return containerListOrder.stream()
