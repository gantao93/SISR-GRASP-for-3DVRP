...
public class VehicleSolver implements SolverInterface {
    private GRASP grasp = new GRASP();
    public VehicleSolver() {
    ...
    public void addPackage(int length, int width, int height, int maxWeight) {
        this.grasp.addContainer().setLength(length).setWidth(width).setHeight(height).setMaxWeight(maxWeight).setContainerType("vehicle");
    }
